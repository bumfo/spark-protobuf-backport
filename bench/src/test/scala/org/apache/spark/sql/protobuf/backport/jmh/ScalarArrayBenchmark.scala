...
/**
 * JMH benchmark for repeated scalar field parsing performance (packed encoding).
 *
 * Compares WireFormatToRowGenerator vs InlineParserToRowGenerator for 4 repeated scalar fields:
 * - repeated int32 (packed varint encoding)
 * - repeated int64 (packed varint encoding)
 * - repeated float (packed fixed32 encoding)
 * - repeated double (packed fixed64 encoding)
 *
 * Parameters:
 ...
 *
 * Usage:
 *   sbt "bench/Jmh/run .*ScalarArrayBenchmark.*"
 *   sbt "bench/Jmh/run .*ScalarArrayBenchmark.* -p size=100 -p field=int32"
 */
...
class ScalarArrayBenchmark {
  ...
  @Setup
  def setup(): Unit = {
    val scalarArrayMsg = TestDataGenerator.createScalarArrayMessage(size)
