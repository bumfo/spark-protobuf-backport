...
public abstract class AbstractWireFormatConverter extends AbstractRowConverter {
     ...
     */
    public InternalRow convert(byte[] binary) throws IOException {
     ...
     */
    protected abstract void writeData(byte[] binary, UnsafeRowWriter writer);
     ...
     */
    protected void writeMessageArray(byte[][] messageBytes, int size, int fieldNumber, int ordinal, UnsafeRowWriter writer) throws IOException {
     ...
     */
    protected void writeMessage(byte[] messageBytes, int fieldNumber, int ordinal, UnsafeRowWriter writer) throws IOException {
        AbstractWireFormatConverter converter = getNestedConverter(fieldNumber);
        if (converter == null) {
            throw new IllegalStateException("No nested converter found for field " + fieldNumber);
        }
